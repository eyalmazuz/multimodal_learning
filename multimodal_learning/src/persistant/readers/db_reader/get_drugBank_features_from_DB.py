...
class GetDrugBankFeaturesFromDB():
    ...
    def get_target_features(self,version, column='uniprot', value_type= 'mapping'):
        """
        column can be target/uniprot
        
        value type can be one of the following:
            - mapping: for number values
            - action: for string values
            - binary: True if action exist, False else
        
        """
        
        table = self.read_table(combine_table_name_version("Target",version))
        table[['target','uniprot']] =table[['target','uniprot']].fillna('None')
        if value_type == 'binary':
            table['binary'] = 1
        
        table = table.pivot_table(index='drugBank_id', columns=column, values=value_type, aggfunc=np.sum)
        if value_type == 'binary':
    ...
    def combine_features(self,sparse_table_name_list,dense_table_name_list,version,add_name_prefix_to_sparse=True,add_counts_to_sparse=False):
        ...
        for t in sparse_table_name_list:
            name = t.replace("_"," ")
            assert name != 'Target', "Target table not supported, please use 'get_target_feature' function"
...
# version = '5.1.1'
# ans = t.get_target_features(version, column='uniprot', value_type= 'binary')
# -
