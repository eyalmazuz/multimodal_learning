...
def get_dataset(old_drug_bank: DrugBank,
    ...
    test_new_old_dataset = TTATestDataset(path=f'{kwargs["data_path"]}/test_new_old.csv',
                               features=features,
                               similar_map_path='./data/jsons/similar_drugs_dict_only_old.json',
                               )

    test_new_new_dataset = TTANNTestDataset(path=f'{kwargs["data_path"]}/test_new_new.csv',
                               features=features,
                               similar_map_path='./data/jsons/similar_drugs_dict_only_old.json',
                               )
...
def get_train_test_pairs(old_drug_bank, new_drug_bank, save_path):
    ...
    if not os.path.exists(f'{save_path}/test_new_old.csv'):
        ...
        smiles_a, smiles_b = zip(*[(new_drug_bank.id_to_drug[drug_a].smiles,
                                old_drug_bank.id_to_drug[drug_b].smiles) for (drug_a, drug_b) in new_old_pairs])

        test_no_vals = list(zip(*zip(*train_drug_pairs), smiles_a, smiles_b, train_labels))
    ...
    if not os.path.exists(f'{save_path}/test_new_new.csv'):
        ...
        test_nb_vals = list(zip(*zip(*new_new_pairs), smiles_a, smiles_b, new_new_labels))
        # for (drug_a, drug_b), label in zip(new_old_pairs, new_old_labels):
        #     smile_a = new_drug_bank.id_to_drug[drug_a].smiles
        #     smile_b = old_drug_bank.id_to_drug[drug_b].smiles
        #     test_no_vals.append((drug_a, smile_a, drug_b, smile_b, label))

        test_nn_df = pd.DataFrame(test_nb_vals, columns=['Drug1_ID', 'Drug2_ID',
                                                            'Drug1_SMILES', 'Drug2_SMILES',
                                                            'label'])
